...
Najprv potrebujete zmeniť tabuľku diskových oddielov, aby zaberal druhý oddiel, `rootfs`, všetko dostupné miesto. Diskový oddiel musíte najskôr zmazať a potom vytvoriť nový. Kľúčové je uistiť sa, že nový, väčší oddiel začína na rovnakom offsete. Možno vám táto operácia príde nebezpečná, ale v skutočnosti nie je. Samozrejme, musíte sa uistiť, že ste si vybrali správne zariadenie. Keď zmeníte tabuľku diskových oddielov, vyberte a znovu zasuňte pamätovú kartu, aby Linux použil aktualizovanú tabuľku. V ďalšom kroku skontrolujte chyby v súborovom systéme a nakoniec spustite príkaz `resize2fs`.
...
# k routru poskytovateľa Internetu.
...
*Aktualizácia 1.11.2020: Nedávno Nadácia Raspberry Pi uviedla [nový výpočtový modul][rpi4-compute] postavený na Raspberry Pi 4. Má rovnaký procesor a väčšinu funkcií RPi4, ale tiež ponúka veci navyše, hlavne port PCIe x1. Pred pár dňami som na Hacker News čítal [príspevok podobný tomu môjmu][rpi4-intel-nic]. Avšak namiesto použitia externého adaptéra z USB na Ethernet použil autor sieťovú kartu od Intelu. Dokázal dosiahnuť priepustnosť 3 Gbit/s, čo je maximum zbernice PCIe x1. Rozbehanie sieťovej karty nebolo vôbec zložité. Použil lacný adaptér na PCIe x16 a zo zdrojových kódov si zostavil ovládač. Myslím si, že stojí za to si to prečítať a je to slušná alternatíva k môjmu prístupu, ak chcete z vášho RPi vyžmýkať ešte viac výkonu.*
