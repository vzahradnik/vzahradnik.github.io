...
excerpt: Ako dobre zvláda Raspberry Pi časovo náročné snímanie dát cez GPIO? Poďme to zistiť.
author: Vladimír Záhradník
date: 2019-06-17
categories: arduino raspberry gpiozero embedded realtime development
...
Raspberry Pi je cenovo veľmi dostupné malé zariadenie, ktoré viete použiť v rôznych projektoch. Má vynikajúcu podporu Linuxu a jeho oficiálna linuxová distribúcia, Raspbian, beží na všetkých podporovaných Raspberry Pi.

Pokiaľ vyvíjate projekt na Raspberry Pi 3 B+ a následne vložíte vašu microSD kartu do Raspberry Pi Zero, je možné, že váš kód bude stále fungovať. Jednou z veľkých výhod je [40-pinový GPIO konektor][rpi-gpio].

Vďaka knižniciam ako [gpiozero][gpiozero] dokážu aj začiatočníci používať GPIO bez komplikácií. Avšak Raspberry Pi nie je mikrokontrolér a Raspbian nie je [systém optimalizovaný na použitie v reálnom čase][rpi-realtime-processing]. Pokiaľ potrebujete vykonávať úlohy kritické na čas, môžete naraziť na problémy, podobne ako sme aj narazili my.

V tomto blogu sa pozrieme na vývoj stopiek pre hasičské súťaže a rozoberieme si niektoré problémy, ktorým sme čelili.
...
Váš callback potrebujete pripojiť do vlastnosti `when_pressed`. Tento komponent pre tlačidlo očakáva funkciu so špecifickými parametrami. Ideálne by ste mu mali dať odkaz na funkciu bez vstupného parametra alebo môžete posunúť funkciu, ktorá očakáva jeden vstupný parameter. Keď bude zavolaná, bude mať správny odkaz na objekt Button.
...
def _update_ui(self):
    ...
    # Tento riadok sa postará o to, že funkcia pre aktualizácie sa bude volať pravidelne,
...
Svojím spôsobom je to podobné k predchádzajúcemu prípadu, pretože taktiež na vstupe dostávame impulzy. Aby sme vypočítali prietok v litroch za sekundu, potrebujeme len inú rovnicu.
...
Zmena napätia sa neprejavila okamžitou zmenou tlaku. Asi by sme mohli viac odladiť výpočet tlaku. Zmena veľkosti cyklického frontu by mohla byť efektívnym riešením. Avšak v našom prípade je to prijateľné.

### Protokolovanie (logovanie) značne spomaľuje snímanie dát
