...
date: 2020-07-26
categories: arduino porting programming development embedded
...
Portovanie softvéru na nové architektúry alebo platformy ma vždy lákalo. Predstavte si, že softvér, ktorý nebol pre vaše zariadenie napísaný, na ňom zrazu beží. Vždy som obdivoval ľudí, ktorí vyvíjajú emulátory pre staršie (ale aj nové konzoly), robia reverzné inžiniérstvo, prípadne napíšu softvér na konzolu bez prístupu k devkitu.
...
Mikrokontrolér nám vie garantovať, že sa kritická úloha bude pravidelne vykonávať, napríklad cez prerušenia. Raspberry Pi môže byť oveľa výkonnejšie, ale aplikácie vyžadujúce beh v reálnom čase dobre nepodporuje. O tom som sa presvedčil už keď sme na ňom robili [stopky pre hasičské súťaže][firefighter-stopwatch] a ukázalo sa, že RPi nie je schopné prijímať stabilný tok impulzov. Niektoré skrátka nezachytilo, pretože pod Linuxom práve bežala na zlomok sekundy iná úloha, a prepočet z impulzov na otáčky motora nám kolísal rádovo o plus/mínus 100 otáčok. Mikrokontrolér nemá nad sebou žiadny operačný systém, váš kód beží priamo na ňom. Preto máte oveľa väčšiu kontrolu, ako zabezpečiť beh kódu kritického na čas.

## Real-time operačné systémy
Samozrejme, existujú aj real-time operačné systémy. Tie sú upravené na beh úloh kritických na čas, pričom programuje sa pre nich jednoduchšie. Máte viac pamäte, dospelejší systém, výkonnejší procesor a plnohodnotnú implementáciu jazyka C/C++, prípadne aj tých vysokoúrovňových ako Rust či Python/MicroPython. Pre Raspberry Pi som však nič aktuálne nenašiel. Vraj by na ňom mal bežať [FreeRTOS][free-rtos] od Amazonu, ale informácie sú ešte pre RPi2. Na najnovšom RPi4 teda nemám žiadnu záruku. Potom je tu projekt, ktorý ma osobne zaujíma už dlhšie, [Zephyr Project][zephyr]. Ten však Raspberry Pi nemá v zozname podporovaných platforiem. Takže to zhrniem, programovanie aplikácií pre real-time operačné systémy má zmysel preskúmať hlbšie a rozhodne sa na to v blízkej dobe pozriem.

## SimpleMotionV2
Potrebujem rozbehať komunikáciu s driverom, ktorý ovláda servomotor. Je dôležité, aby komunikácia prebiehala veľmi rýchlo, niekoľko sto krát za sekundu, aby som vedel prečítať reálne údaje z motora a aby som stihol pohyb motora korigovať, ak vyhodnotím, že pohyb sa vychyľuje z normy. Používame driver [IONI][ioni-drive] od firmy Granite Devices. Komunikácia prebieha po zbernici RS485, pričom firma vyvinula vlastný komunikačný protokol, SimpleMotion. RS485 totiž definuje len fyzické parametre prenosovej linky a je na vás, ako budete informácie prenášať.
...
Najlepší zdroj informácií mi paradoxne poskytol zdrojový kód knižnice, a to nie je bohvie čo. V jej repozitári som našiel podadresár `doc`, kde bol uvedený popis protokolu. Zjednodušene, potom, ako otvoríte RS485 linku, komunikujete so zariadeniami asymetricky v modeli master/slave. Váš počítač, respektíve mikrokontrolér, je master. Komunikuje s jedným, alebo viacerými slave zariadeniami. Všetky zariadenia sú pripojené na tú istú zbernicu a každé zariadenie má priradenú jedinečnú adresu uzla. Vy, ako master, viete parametre čítať alebo zapisovať. V oboch prípadoch potrebujete vedieť názov parametra a adresu uzla.
...
### Nutné súbory
- simplemotion.c/.h - Jadro celej knižnice. Definuje API, ktoré volajú používatelia z ich kódu, napríklad funkciu `smOpenBus( const char * devicename )`, ktorá inicializuje komunikáciu, alebo funkciu `smRead1Parameter( ... )`, ktorá prečíta zo slave zariadenia nejakú hodnotu
...
### Nepovinné súbory
- bufferedmotion.c/.h - Kód, ktorý pridáva možnosť posielať príkazy do driveru v dávkach. Driver si ich uloží do zásobníka a postupne ich vykoná
...
Knižnicu samozrejme ešte nemáte v registri Arduino knižníc. Arduino vám umožňuje nainštalovať cez IDE `.zip` archív s kódom knižnice, alebo si viete knižnicu nakopírovať sami. Arduino IDE hľadá knižnice v konkrétnych adresároch, v mojom prípade:
```bash
...
Je úplne jedno, ako pomenujete adresár s knižnicou. Arduino IDE hľadá hlavičkové súbory vo všetkých týchto adresároch.
...
Autori tejto knižnice zároveň pripravili ovládače pre najbežnejšie platformy. Tie zabezpečia komunikáciu po RS485 cez sériovú linku, prípadne cez USB prevodník s čipom FTDI. Sú platformovo špecifické, fungujú pre Windows, Linux a macOS. Štandardne sú tieto ovládače zakázané a používateľ ich musí povoliť pri kompilácii flagom `ENABLE_BUILT_IN_DRIVERS`. V takom prípade si používateľ zavolá funkciu `smOpenBus( const char * devicename )`, kde jej zadá ako parameter názov portu (povedzme `/dev/ttyS0`). Kód sa postupne snaží volať jednotlivé ovládače a pokiaľ niektorý z nich nevráti chybu, komunikácia sa považuje za úspešnú. Tieto ovládače priamo v sebe implementujú tieto štyri základné funkcie konkrétne pre ich cieľovú architektúru.

Alternatívou je implementovať tieto callback funkcie priamo vo vašom kóde. V takom prípade zavoláte vo vašom kóde funkciu `smOpenBusWithCallbacks( ... )`, ktorej definujete pomocou parametrov referencie na tieto callback funkcie. Aj keď je tento spôsob funkčný a úspešne som ho otestoval s mojím [Controllinom][controllino], keďže robíme Arduino knižnicu, môžeme si dovoliť začleniť kód na inicializáciu zbernice priamo do knižnice v podobe nového ovládača. Zatiaľ som implementoval jednoduchý ovládač pre priemyslové Controllino Maxi/Mega, ktoré práve používam.

Ak sa nám doteraz knižnicu darilo úspešne skompilovať, po povolení flagu `ENABLE_BUILT_IN_DRIVERS` sa to zmení. Zrazu sa začnú kompilovať všetky ovládače z priečinka `drivers` a kým kompilátor poskytoval niektoré základné hlavičkové súbory štandardného C-čka, u ovládačov už narazíte. Pracujú napríklad so socketmi, majú volania na `pthread`, a tak ďalej. Najprv som skúšal upraviť kód pomocou `#ifdef ARDUINO #endif` blokov, len aby kód skompiloval. Ukázalo sa, že kód je už taký špecifický, že to nemá cenu. Nakoniec som spravil niečo podobné ako pri súbore `devicedeployment.h` vyššie. Zablokoval som všetok existujúci kód pre každý ovládač, no tentokrát som musel zachovať aspoň základnú implementáciu callbackov. Tie sa totiž volajú v knižnici a tá očakáva, že také funkcie existujú. Na Arduine sa funkcie úspešne zavolajú, ale vrátia pri inicializácii ovládača chybu. Inak je telo funkcií prázdne.
...
Po tejto úprave už kompilácia opäť bežala. Mohol som sa pustiť do vytvorenia ovládača pre RS485 komunikáciu pre Controllino. Ako som už napísal, základom je implementovať zopár funkcií, ktoré si knižnica zavolá. Controllino má ukážkový príklad pre RS485, komunikácia sa dá rozbehať na pár riadkoch kódu. Pre predstavu, takto jednoducho zabezpečím čítanie dát:
...
Tento problém som sa rozhodol vyriešiť tak, že všetok C++ kód pre Arduino som začlenil do samostatného modulu, ktorého API sa dá volať aj z C-čkovského kódu. Vytvoril som modul `arduino_helper.h/.c` a umiestnil som ho priamo k ovládaču pre Controllino. Celkovo sa moje zmeny v knižnici dosť sprehľadnili. Pôvodný súbor `simplemotion.c` má minimum zmien a všetko dôležité sa deje v tomto module.
...
Prvý význam: Ak ho uvediete u premennej, sprístupníte ju aj ostatným modulom. Zvyčajne takéto premenné umiestníte do hlavičkového súboru, aby si ich mohli ľahko sprístupniť ostatné moduly.
...
Druhý význam: Ak ho uvediete pri deklarácii funkcie, sprístupníte ju aj ostatným modulom. U funkcií platí, že `extern` je uvedený implicitne a nemusíte ho uvádzať. Aj tu platí, že názov funkcie musí byť v celej knižnici jedinečný. C-čko nepozná menné priestory.
...
V praxi sa tento blok pridáva cez direktívy preprocesora. Ak si hlavičkový súbor načíta modul napísaný v C++, vie, že funkcie majú názvy kompatibilné s C-čkovským kódom. A modul napísaný v C-čku direktívu extern vôbec neuvidí.
...
Ak máte niekde v kóde funkcie, ktoré očakávajú ako parameter smerník na objekt, takýto kód sa neskompiluje. Pre C-čkovské moduly je pojem trieda neznámy a vyhlásia pri tomto type chybu. Momentálne som problém obišiel tak, že som zadefinoval funkciu, ktorá očakáva smerník na blok `void` a neskôr v C++ kóde si tento smerník pretypujem na C++ objekt. Viem, že to nie je dokonalé, ale funguje to.
...
Ten sa pretypuje na `ArduinoSerial`, prelezie cez C-čkovský kód až do `arduino_helper` modulu, kde si ho pretypujem na smerník na objekt `Print`. Následne už môžem zapisovať na sériovú linku, ako som zvyknutý.
```cpp
/*
 * arduino_helper.cpp
 *
...
## Makrá a upozornenia
Aj keď mi kompilácia zbehne, všimol som si množstvo chýb v makrách. Analýzou som zistil, že makrá sa používajú len v troch funkciách určených prevažne na debugovanie. Odnastavením flagu `ENABLE_DEBUG_PRINTS` sa väčšiny upozornení zbavím. V tejto chvíli ešte neviem, či je to problém. To ukáže až ďalšie testovanie.
...
U PlatformIO však narážam na iný problém. Driver IONI nemám u seba, ale je pripojený na jedno Raspberry Pi asi 500 kilometrov ďaleko. Mám naňom rozbehanú vzdialenú plochu cez VNC. Arduino IDE je na Raspberry Pi podporované, ale PlatformIO už nie. Najjednoduchšie riešenie mi príde skompilovať firmvér lokálne a nejako ho nahrať cez Raspberry Pi priamo na vývojovú dosku.
...
- Všetok zdrojový kód musí ísť do podadresára `src`
- Je potrebné napísať príklady použitia do adresára `examples`
...
- Bolo by tiež vhodné nastaviť automatizované spúšťanie zostavy pre Arduino dosky cez Travis CI s využitím testovacieho frameworku [Arduino CI][arduino-ci]

Fork som vytvoril pod organizáciou, s ktorou spolupracujem. Nájdete ho [tu][simplemotion-fork]. V tejto chvíli ešte neviem, do akej miery ho budem udržiavať. Rád by som však knižnicu odovzdal komunite dostatočne funkčnú, aby fungovala pod Arduino IDE aj PlatformIO. Keďže zatiaľ mám podporu hlavne pre Controllino, dal by sa implementovať aj ovládač pre Arduino RS485 rozširujúcu kartu (hat), prípadne nejaké ďalšie funkcie.

## Ďalšie kroky
V tejto chvíli viem, že základná inicializácia funguje. Ostáva skúsiť vymeniť si prvé správy s IONI driverom a ak to nebude fungovať, je potrebné zistiť prečo. Ďalej je potrebné upraviť manifest knižnice, aby obsahoval všetky aktuálne informácie o našom projekte, odstrániť chyby spojené s kompiláciou a verím, že knižnica bude dobre slúžiť.
